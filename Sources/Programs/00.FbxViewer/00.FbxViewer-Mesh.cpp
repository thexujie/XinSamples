...
int FbxViewer_Main(IRHIDeviceRef Device)
{
	float32 FovY = Float32PI * 0.333333f;

	IBaseEngineRef Engine = new FEngine(*Device);
	...
	IRHIViewportRef Viewport = Device->CreateViewport(CommandExecutor.PrimaryQueue, WindowHandle, ClientSize, NumBackBuffers, EFormat::R8G8B8A8UF, EPresentMode::FlipDiscard);
	IRHIResourceRef DepthStencil = CommandExecutor.CreateTexture({ EFormat::D32F, ClientSize, EResourceUsage::DepthStencil, { 1.0f, 0 } });

	IShaderInstance * VertexShader = Engine->GetShaderT<FStaticMeshVS>();
	//VertexShader->SetResourceLabel(u8"MainVS"V);
	IShaderInstance * PixelShader = Engine->GetShaderT<FStaticMeshPS>();
	...
	IRHIPipelineStateRef PipelineState = Device->CreatePipelineState(PipelineStateDesc);
	...
	FApplication::Instance().Loop([&]
	{
		...
		uintx BackBufferIndex = Viewport->BeginFrame();
