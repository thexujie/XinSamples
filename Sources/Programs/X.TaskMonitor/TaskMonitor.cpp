...
namespace Xin::TaskMonitor
{
	...
	static void AdjustPrivileges()
	{
		...
		if (::OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &ProcessToken))
		{
			...
			AssertExpresion(AsjustSucceeded);
		}
	}
		...
		{
			{
				...
				[](const UListViewItemRef & A, const UListViewItemRef & B)
				{
					...
					int32 Result = StringCompareIC<Char>(ProcessItemA.ProcessName, ProcessItemB.ProcessName);
			...
			},
			{
				...
				[](const UListViewItemRef & A, const UListViewItemRef & B)
				{
					FProcessItem & ProcessItemA = A->Content.Get<FProcessItem&>();
					FProcessItem & ProcessItemB = B->Content.Get<FProcessItem&>();
					AssertExpresion(!IsNaN(ProcessItemA.CPUUsage));
					AssertExpresion(!IsNaN(ProcessItemB.CPUUsage));
			...
			},
			{
				...
				[](const UListViewItemRef & A, const UListViewItemRef & B)
				{
					FProcessItem & ProcessItemA = A->Content.Get<FProcessItem&>();
					FProcessItem & ProcessItemB = B->Content.Get<FProcessItem&>();
					int32 Result = StringCompareIC<Char>(ProcessItemA.CurrentDirectory, ProcessItemB.CurrentDirectory);
					return Result ? (Result < 0) : (ProcessItemA.ProcessId < ProcessItemB.ProcessId);
				}
			},
			{
				...
				[](const UListViewItemRef & A, const UListViewItemRef & B)
				{
					FProcessItem & ProcessItemA = A->Content.Get<FProcessItem&>();
					FProcessItem & ProcessItemB = B->Content.Get<FProcessItem&>();
					int32 Result = StringCompareIC<Char>(ProcessItemA.Description, ProcessItemB.Description);
					return Result ? (Result < 0) : (ProcessItemA.ProcessId < ProcessItemB.ProcessId);
				}
			},
			{
				...
				[](const UListViewItemRef & A, const UListViewItemRef & B)
				{
					FProcessItem & ProcessItemA = A->Content.Get<FProcessItem&>();
					FProcessItem & ProcessItemB = B->Content.Get<FProcessItem&>();
					int32 Result = StringCompareIC<Char>(ProcessItemA.CompanyName, ProcessItemB.CompanyName);
					return Result ? (Result < 0) : (ProcessItemA.ProcessId < ProcessItemB.ProcessId);
				}
			},
			{
				...
				[](const UListViewItemRef & A, const UListViewItemRef & B)
				{
					FProcessItem & ProcessItemA = A->Content.Get<FProcessItem&>();
					FProcessItem & ProcessItemB = B->Content.Get<FProcessItem&>();
					int32 Result = StringCompareIC<Char>(ProcessItemA.ProductName, ProcessItemB.ProductName);
					return Result ? (Result < 0) : (ProcessItemA.ProcessId < ProcessItemB.ProcessId);
				}
			},
			{
				...
				[](const UListViewItemRef & A, const UListViewItemRef & B)
				{
					FProcessItem & ProcessItemA = A->Content.Get<FProcessItem&>();
					FProcessItem & ProcessItemB = B->Content.Get<FProcessItem&>();
					int32 Result = StringCompareIC<Char>(ProcessItemA.FileVersion, ProcessItemB.FileVersion);
					return Result ? (Result < 0) : (ProcessItemA.ProcessId < ProcessItemB.ProcessId);
				}
			},
		};
	...
	void FTaskMonitor::UpdateProcesses()
	{
		...
		AssertExpresion(TotalCycleTime > 0);
		for (FProcessItemRef & ProcessItem : ProcessItems)
		{
