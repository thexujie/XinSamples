...
namespace SDK
{
	...
	void FSDKMesh::LoadFromFile(FStringV FilePath)
	{
		...
		for (UINT VertexBufferIndex = 0; VertexBufferIndex < MeshHeader.NumVertexBuffers; VertexBufferIndex++)
		{
			...
			AssertExpresion(SDKVertexBufferHeader.SizeBytes == SDKVertexBufferHeader.StrideBytes * SDKVertexBufferHeader.NumVertices);
		...
		for (UINT IndexBufferIndex = 0; IndexBufferIndex < MeshHeader.NumIndexBuffers; IndexBufferIndex++)
		{
			...
			AssertExpresion(StaticMeshData.IndexFormat == EIndexFormat::None || StaticMeshData.IndexFormat == IndexFormat);
		...
		for (UINT MeshIndex = 0; MeshIndex < MeshHeader.NumMeshes; MeshIndex++)
		{
			...
			AssertExpresion(SDKMesh.NumVertexBuffers == 1);
			...
			for (UINT SubsetIndex = 0; SubsetIndex < SDKMesh.NumSubsets; SubsetIndex++)
			{
				...
				AssertExpresion(SDKSubset.PrimitiveType == SDKMESH_PRIMITIVE_TYPE::PT_TRIANGLE_LIST);
				...
				AssertExpresion(SDKSubset.MaterialID != INVALID_MATERIAL);
